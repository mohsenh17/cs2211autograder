...
def getStudentFiles(directoryAddress):
    """
    Extracts student submission files for assignment 4 from the given directory address.
    ...
    for submission in glob.glob('{}/*'.format(directoryAddress)):
        ...
        try:
            poc = subprocess.run("tar -xf {}/25*-Assignment4.tar -C subs/{}".format(submission, studentName), shell=True, capture_output=True, text=True, check=True)
